...
class SpectrometerApp:
    ...
    def setup_ui(self):
        ...
        graph_window_button.grid(row=1, column=0, padx=5, pady=5)
...
class GraphWindow(tk.Toplevel):
    def __init__(self, app):
        ...
        average_label.grid(row=1, column=0, padx=5, pady=5, sticky='w')

        self.average_var = tk.IntVar(value=1)
        self.average_slider = ttk.Scale(control_frame, from_=1, to=10, orient=tk.HORIZONTAL,
                                        command=self.update_average)
        self.average_slider.set(1)
        self.average_slider.grid(row=1, column=1, padx=5, pady=5, sticky='w')
    ...
    def update_average(self, value):
        self.average_points = int(float(value))
        if self.update_flag:
            self.update_plot()
    ...
    def update_plot(self):
        if self.update_flag:
            ...
            if len(self.app.data) >= 3:
                ...
                norm = np.interp(averaged_data, (min(averaged_data), max(averaged_data)), (0, 1))
                inverted_data = 1 - norm

                # Create plotly figure
                fig = go.Figure()

                # Add the inverted intensity line to the figure
                fig.add_trace(go.Scatter(x=x_data, y=inverted_data, mode='lines', name='Intensity',
                ...
                # Set x-axis ticks for wavelength or frequency
                if self.mode == 'wavelength':
                    fig.update_xaxes(tickvals=np.linspace(300, 900, 7))
                else:
                    fig.update_xaxes(tickvals=np.linspace(min(x_data), max(x_data), 7))
                ...
                fig.add_annotation(
                    x=max_x, y=max_value,
                    text=f'Peak: {max_x:.1f} {x_label.split()[1]}',
                    showarrow=True,
                    arrowhead=2,
                    ax=50, ay=-40
                )
                ...
                # Render the plot
                self.plotly_canvas.show_plot(fig)
            else:
